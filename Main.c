...
void aboutus()
{
    ...
    printf("The site near Jatni Railway Station on the East Coast Railway is 35 kilometres from Biju Patnaik Airport Bhubaneswar. \nThe land is picturesque with Barunai Hill on one side and a reserve forest nearby. A total of 936 acres (3.79 km2) of land has been allocated towards the self-contained campus for nearly 10,000 students and 1,100 faculty. A science park is being set up as a part of this hotel to support industry oriented research activities. The institute has started operating in its new campus at Arugul from academic year 2015/16. The campus construction began on 14 August 2011, Chief minister Naveen Patnaik laid the foundation stone of the IIT campus at Aragul village near Jatni. The campus master plan has been prepared by Vastunidhi Architects, Noida.");
	printf("\n\n\n                          -------------OUR DETAILS------------");
	printf("\n\n");
...
void edit()
{
	...
	while(fread(&s,sizeof(s),1,f)==1)
	{
		if(strcmp(s.roomnumber,roomnumber)==0)
		{
			...
			printf("\nEnter Arrival date :");
			scanf("%s",&s.arrivaldate);
			fseek(f,size,SEEK_CUR);  //to go to desired position
			fwrite(&s,sizeof(s),1,f);
			break;
		}
	}
...
void GuessGame(int amount_bet, int *inhand_cash)
{
    ...
    int PlayerGuess;
    printf("\nYou may now guess the position in which R is present: ");
    scanf("%d", &PlayerGuess);
...
void mini_casino()
{
	system("cls");
	setcolor(3);
    int amount_bet, inhand_cash,a,count=0,m;
...
printf("\n----Enter the inhand cash you have right now---- :\n ");
